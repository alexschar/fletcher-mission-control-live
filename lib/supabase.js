...
async function getLifeSignals(filters = {}) {
  ...
  if (filters.source) {
    // Support both exact match and prefix match (e.g., source=instagram matches instagram, instagram_alexcandothat, etc.)
    if (filters.source_prefix === true || filters.source_prefix === 'true') {
      console.log(`[DEBUG] Using prefix filter: source like '${filters.source}%'`);
      query = query.like('source', `${filters.source}%`);
    } else {
      console.log(`[DEBUG] Using exact filter: source = '${filters.source}'`);
