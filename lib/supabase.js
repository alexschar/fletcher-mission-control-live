...
async function getInteractMessages(filters = {}) {
  let query = supabase.from('interact_messages').select('*');

  if (filters.status) query = query.eq('status', filters.status);
  if (filters.agent_target) query = query.eq('agent_target', filters.agent_target);
...
async function createLifeSignal(signal) {
  const payload = {
    source: signal.source,
    category: signal.category,
    signal_type: signal.signal_type,
    title: signal.title,
    body: signal.body ? String(signal.body).slice(0, 2000) : null,
    metadata: parseJsonField(signal.metadata, {}),
    priority: signal.priority || 'normal',
    status: signal.status || 'unread',
    agent_notes: parseJsonField(signal.agent_notes, null),
    agent_draft: signal.agent_draft || null,
  };
...
async function getLifeSignals(filters = {}) {
  let query = supabase.from('life_signals').select('*');
  ...
  if (filters.priority) query = query.eq('priority', filters.priority);
  if (filters.since) query = query.gte('created_at', filters.since);
  if (filters.has_feedback === 'true') query = query.not('feedback', 'is', null);
  if (filters.has_feedback === 'false') query = query.is('feedback', null);
  if (filters.exclude_dismissed) query = query.neq('status', 'dismissed');

  query = query.order('created_at', { ascending: false });
  query = query.limit(filters.limit ? parseInt(filters.limit, 10) : 50);

  const { data, error } = await query;
  if (error) throw error;
  return (data || []).map(normalizeSignalJsonFields);
}

function normalizeSignalJsonFields(signal) {
  if (!signal) return signal;
  signal.metadata = parseJsonField(signal.metadata, {});
  signal.agent_notes = parseJsonField(signal.agent_notes, null);
  return signal;
}

async function getLifeSignalById(id) {
  const { data, error } = await supabase
    .from('life_signals')
    .select('*')
    .eq('id', id)
  ...
  if (error) throw error;
  return normalizeSignalJsonFields(data);
}

async function updateLifeSignal(id, updates) {
  const allowed = [
    'status', 'priority', 'agent_notes', 'agent_draft',
    'feedback', 'feedback_note', 'processed_at'
  ];
  const payload = {};
  ...
  // Auto-set processed_at when status changes to read/acted_on
  if (payload.status && ['read', 'acted_on'].includes(payload.status) && !payload.processed_at) {
...
async function getLifeSignalStats() {
  const [totalResult, unreadResult, feedbackResult, latestResult] = await Promise.all([
    supabase.from('life_signals').select('*', { count: 'exact', head: true }),
    supabase.from('life_signals').select('*', { count: 'exact', head: true }).eq('status', 'unread'),
    supabase.from('life_signals').select('*', { count: 'exact', head: true }).not('feedback', 'is', null),
    supabase.from('life_signals').select('created_at').order('created_at', { ascending: false }).limit(1),
  ]);
  ...
  // Get recent feedback for preference learning stats
  ...
  const feedbackBreakdown = (recentFeedback || []).reduce((acc, row) => {
    acc[row.feedback] = (acc[row.feedback] || 0) + 1;
    return acc;
  }, {});

  const latestSignalAt = latestResult.data?.[0]?.created_at || null;

  return {
    total: totalResult.count || 0,
    unread: unreadResult.count || 0,
    with_feedback: feedbackResult.count || 0,
    feedback_this_week: feedbackBreakdown,
...
// ==================== AGENT ACTIONS ====================
async function createAgentAction(action) {
  const payload = {
    agent: action.agent,
    action: action.action_type || action.action,
    target: action.target || null,
    status: action.status || 'completed',
    metadata: action.metadata || action.details || {},
...
async function getAgentActions(filters = {}) {
...
async function getAgentActionStats(period = 'week') {
  const cutoff = getCutoffDate(period);
  const { data, error } = await supabase
    .from('agent_actions')
    .select('*')
  ...
  actions.forEach(action => {
    byAgent[action.agent] = (byAgent[action.agent] || 0) + 1;
    byType[action.action] = (byType[action.action] || 0) + 1;
  });
...
// ==================== AGENT LEARNINGS ====================
async function createAgentLearning(learning) {
  const payload = {
    agent: learning.agent,
    category: learning.category,
    content: learning.content || learning.title,
    verified: learning.verified || false,
    usage_count: learning.applied_count || learning.usage_count || 0,
...
async function getAgentLearnings(filters = {}) {
  let query = supabase.from('agent_learnings').select('*');

  if (filters.agent) query = query.eq('agent', filters.agent);
  if (filters.category) query = query.eq('category', filters.category);
  if (filters.verified === true || filters.verified === 'true') query = query.eq('verified', true);
  if (filters.verified === false || filters.verified === 'false') query = query.eq('verified', false);
  if (filters.search) {
    const search = filters.search.trim();
    query = query.or([
      `title.ilike.%${search}%`,
...
async function updateAgentLearning(id, updates) {
  const allowed = ['content', 'verified', 'usage_count'];
  const payload = {};
  for (const key of allowed) {
    if (key in updates) payload[key] = updates[key];
  }
  // Map applied_count to usage_count for compatibility
  if ('applied_count' in updates) {
    payload.usage_count = updates.applied_count;
  }
...
async function incrementLearningAppliedCount(id) {
...
async function getAgentLearningStats() {
  const [{ count: total, error: totalError }, { count: verified, error: verifiedError }, byAgentResult] = await Promise.all([
    supabase.from('agent_learnings').select('*', { count: 'exact', head: true }),
    supabase.from('agent_learnings').select('*', { count: 'exact', head: true }).eq('verified', true),
    supabase.from('agent_learnings').select('agent')
  ]);

  if (totalError) throw totalError;
  if (verifiedError) throw verifiedError;
  if (byAgentResult.error) throw byAgentResult.error;

  const byAgent = (byAgentResult.data || []).reduce((acc, row) => {
