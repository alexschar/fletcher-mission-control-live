...
async function updateAgentLearning(id, updates) {
  const allowed = ['title', 'content', 'verified', 'applied_count'];
  const payload = {};
  for (const key of allowed) {
    if (key in updates) payload[key] = updates[key];
  }
...
async function incrementLearningAppliedCount(id) {
  ...
  const newCount = (existing?.applied_count || 0) + 1;

  const { data, error } = await supabase
    .from('agent_learnings')
    .update({ applied_count: newCount, updated_at: new Date().toISOString() })
    .eq('id', id)
    .select()
    .single();
