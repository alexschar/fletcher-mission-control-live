// JSON Fallback Module
// Provides the same interface as supabase.js but stores data in JSON files
// Used when Supabase tables don't exist yet

const fs = require('fs');
const path = require('path');

// Use process.cwd() for correct path resolution in both dev and build
const DATA_DIR = path.join(process.cwd(), 'data');

// Ensure data directory exists
if (!fs.existsSync(DATA_DIR)) {
  fs.mkdirSync(DATA_DIR, { recursive: true });
}

// File paths
const FILES = {
  tasks: path.join(DATA_DIR, 'tasks.json'),
  conversations: path.join(DATA_DIR, 'conversations.json'),
  costs: path.join(DATA_DIR, 'costs.json'),
  agent_status: path.join(DATA_DIR, 'agent_statuses.json'),
  override_log: path.join(DATA_DIR, 'override_log.json'),
  project_status: path.join(DATA_DIR, 'project_status.json'),
  agent_actions: path.join(DATA_DIR, 'agent_actions.json'),
  agent_learnings: path.join(DATA_DIR, 'agent_learnings.json')
};

// Helper: Read JSON file (create empty array/object if doesn't exist)
function readJSON(filePath, defaultValue = []) {
  try {
    if (!fs.existsSync(filePath)) {
      fs.writeFileSync(filePath, JSON.stringify(defaultValue, null, 2));
      return defaultValue;
    }
    const data = fs.readFileSync(filePath, 'utf-8');
    return JSON.parse(data);
  } catch (error) {
    console.error(`Error reading ${filePath}:`, error.message);
    return defaultValue;
  }
}

// Helper: Write JSON file
function writeJSON(filePath, data) {
  try {
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
    return true;
  } catch (error) {
    console.error(`Error writing ${filePath}:`, error.message);
    return false;
  }
}

// Helper: Generate ID
function generateId() {
  return Date.now().toString() + Math.random().toString(36).substr(2, 9);
}

// ==================== TASKS ====================

async function createTask(task) {
  const tasks = readJSON(FILES.tasks, []);
  const newTask = {
    id: generateId(),
    title: task.title,
    description: task.description,
    status: task.status || 'backlog',
    assigned_to: task.assigned_to,
    created_by: task.created_by,
    priority: task.priority || 'medium',
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString()
  };
  tasks.unshift(newTask);
  writeJSON(FILES.tasks, tasks);
  return newTask;
}

async function updateTask(id, updates) {
  const tasks = readJSON(FILES.tasks, []);
  const index = tasks.findIndex(t => t.id === id);
  if (index === -1) throw new Error('Task not found');
  
  tasks[index] = { ...tasks[index], ...updates, updated_at: new Date().toISOString() };
  writeJSON(FILES.tasks, tasks);
  return tasks[index];
}

async function deleteTask(id) {
  const tasks = readJSON(FILES.tasks, []);
  const filtered = tasks.filter(t => t.id !== id);
  writeJSON(FILES.tasks, filtered);
}

async function getTasks(filters = {}) {
  let tasks = readJSON(FILES.tasks, []);
  
  if (filters.status) {
    tasks = tasks.filter(t => t.status === filters.status);
  }
  if (filters.assigned_to) {
    tasks = tasks.filter(t => t.assigned_to === filters.assigned_to);
  }
  if (filters.created_by) {
    tasks = tasks.filter(t => t.created_by === filters.created_by);
  }
  if (filters.priority) {
    tasks = tasks.filter(t => t.priority === filters.priority);
  }
  
  // Sort by created_at descending
  tasks.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  
  return tasks;
}

// ==================== CONVERSATIONS ====================

async function addConversationSummary(summary) {
  const conversations = readJSON(FILES.conversations, []);
  const newConversation = {
    id: generateId(),
    agent: summary.agent,
    summary: summary.summary,
    topics: summary.topics || [],
    source: summary.source,
    created_at: new Date().toISOString()
  };
  conversations.unshift(newConversation);
  writeJSON(FILES.conversations, conversations);
  return newConversation;
}

async function getRecentConversations(hours = 24) {
  const conversations = readJSON(FILES.conversations, []);
  const cutoff = new Date(Date.now() - hours * 60 * 60 * 1000);
  
  return conversations
    .filter(c => new Date(c.created_at) >= cutoff)
    .sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
}

// ==================== COSTS ====================

async function addCostEntry(entry) {
  const costs = readJSON(FILES.costs, []);
  const newEntry = {
    id: generateId(),
    agent: entry.agent,
    model: entry.model,
    provider: entry.provider,
    tokens_in: entry.tokens_in || entry.input_tokens || 0,
    tokens_out: entry.tokens_out || entry.output_tokens || 0,
    cost_est: entry.cost_est || entry.calculated_cost || 0,
    notes: entry.notes,
    timestamp: entry.timestamp || new Date().toISOString()
  };
  costs.unshift(newEntry);
  writeJSON(FILES.costs, costs);
  return newEntry;
}

async function getCosts(period = null) {
  let costs = readJSON(FILES.costs, []);
  
  if (period) {
    const cutoff = getCutoffDate(period);
    if (cutoff) {
      costs = costs.filter(c => new Date(c.created_at) >= cutoff);
    }
  }
  
  return costs.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
}

function getCutoffDate(period) {
  const now = new Date();
  switch (period) {
    case 'day':
      return new Date(now - 24 * 60 * 60 * 1000);
    case 'week':
      return new Date(now - 7 * 24 * 60 * 60 * 1000);
    case 'month':
      return new Date(now - 30 * 24 * 60 * 60 * 1000);
    default:
      return null;
  }
}

// ==================== AGENT STATUS ====================

async function updateAgentStatus(agent, status) {
  const statuses = readJSON(FILES.agent_status, {});
  
  statuses[agent] = {
    agent,
    ...status,
    updated_at: new Date().toISOString()
  };
  
  writeJSON(FILES.agent_status, statuses);
  return statuses[agent];
}

async function getAgentStatus() {
  const statuses = readJSON(FILES.agent_status, {});
  
  // Add agent property from the key if not present
  const values = Object.entries(statuses).map(([key, value]) => ({
    ...value,
    agent: value.agent || key
  }));
  
  return values.sort((a, b) => a.agent.localeCompare(b.agent));
}

// ==================== OVERRIDE LOG ====================

async function addOverrideLog(entry) {
  const logs = readJSON(FILES.override_log, []);
  const newLog = {
    id: generateId(),
    tier: entry.tier,
    task_description: entry.task_description,
    risk_level: entry.risk_level,
    outcome: entry.outcome,
    details: entry.details,
    created_at: new Date().toISOString()
  };
  logs.unshift(newLog);
  writeJSON(FILES.override_log, logs);
  return newLog;
}

async function getOverrides(limit = 50) {
  const logs = readJSON(FILES.override_log, []);
  return logs.slice(0, limit);
}

// ==================== MEMORY FILES ====================

const MEMORY_FILES_PATH = path.join(DATA_DIR, 'memory_files.json');

async function upsertMemoryFile(file) {
  const files = readJSON(MEMORY_FILES_PATH, []);
  const existingIndex = files.findIndex(f => f.name === file.name);
  
  const newFile = {
    name: file.name,
    path: file.path || 'memory',
    content: file.content,
    agent: file.agent || 'system',
    size: file.content?.length || 0,
    updated_at: new Date().toISOString(),
    modified: new Date().toISOString()
  };
  
  if (existingIndex >= 0) {
    files[existingIndex] = newFile;
  } else {
    files.unshift(newFile);
  }
  
  writeJSON(MEMORY_FILES_PATH, files);
  return newFile;
}

async function getMemoryFiles(agent = null) {
  const files = readJSON(MEMORY_FILES_PATH, []);
  if (agent) {
    return files.filter(f => f.agent === agent);
  }
  return files.sort((a, b) => new Date(b.updated_at) - new Date(a.updated_at));
}

async function getMemoryFile(name) {
  const files = readJSON(MEMORY_FILES_PATH, []);
  return files.find(f => f.name === name) || null;
}

async function deleteMemoryFile(name) {
  const files = readJSON(MEMORY_FILES_PATH, []);
  const filtered = files.filter(f => f.name !== name);
  writeJSON(MEMORY_FILES_PATH, filtered);
}

// ==================== PROJECT STATUS ====================

const PROJECT_STATUS_SEED = [
  {
    id: generateId(),
    project_name: 'Neon Pastoral',
    status: 'active',
    last_update: 'Unity optimization phases 2-3 complete, enemy AI gameplay fixes in progress',
    updated_at: new Date(Date.now() - 4 * 60 * 1000).toISOString(),
    updated_by: 'Alex'
  },
  {
    id: generateId(),
    project_name: 'Mission Control',
    status: 'active',
    last_update: 'agent dashboard, project page being built',
    updated_at: new Date(Date.now() - 3 * 60 * 1000).toISOString(),
    updated_by: 'Celeste'
  },
  {
    id: generateId(),
    project_name: 'Portfolio',
    status: 'active',
    last_update: 'software portfolio site in progress',
    updated_at: new Date(Date.now() - 2 * 60 * 1000).toISOString(),
    updated_by: 'Alex'
  },
  {
    id: generateId(),
    project_name: 'Content Pipeline',
    status: 'active',
    last_update: 'Telegram bot running, social metrics polling',
    updated_at: new Date(Date.now() - 1 * 60 * 1000).toISOString(),
    updated_by: 'Sawyer'
  },
  {
    id: generateId(),
    project_name: 'OpenClaw Agent System',
    status: 'active',
    last_update: '3 agents operational, operating model reset today',
    updated_at: new Date().toISOString(),
    updated_by: 'Fletcher'
  }
];

async function ensureProjectStatusSeeded() {
  const projects = readJSON(FILES.project_status, []);
  if (projects.length > 0) return projects.length;
  writeJSON(FILES.project_status, PROJECT_STATUS_SEED);
  return PROJECT_STATUS_SEED.length;
}

async function getProjectStatuses() {
  const projects = readJSON(FILES.project_status, []);
  return projects.sort((a, b) => new Date(b.updated_at) - new Date(a.updated_at));
}

async function updateProjectStatusById(id, updates) {
  const projects = readJSON(FILES.project_status, []);
  const index = projects.findIndex(project => project.id === id);
  if (index === -1) throw new Error('Project not found');

  projects[index] = {
    ...projects[index],
    last_update: updates.last_update,
    updated_by: updates.updated_by,
    updated_at: new Date().toISOString(),
  };

  writeJSON(FILES.project_status, projects);
  return projects[index];
}

// ==================== AGENT ACTIONS ====================

async function createAgentAction(action) {
  const actions = readJSON(FILES.agent_actions, []);
  const newAction = {
    id: generateId(),
    agent: action.agent,
    action_type: action.action_type,
    description: action.description || null,
    target: action.target || null,
    status: action.status || 'completed',
    metadata: action.metadata || {},
    created_at: action.created_at || new Date().toISOString()
  };
  actions.unshift(newAction);
  writeJSON(FILES.agent_actions, actions);
  return newAction;
}

async function getAgentActions(filters = {}) {
  let actions = readJSON(FILES.agent_actions, []);

  if (filters.agent) actions = actions.filter(a => a.agent === filters.agent);
  if (filters.action_type) actions = actions.filter(a => a.action_type === filters.action_type);
  if (filters.status) actions = actions.filter(a => a.status === filters.status);
  if (filters.target) actions = actions.filter(a => a.target === filters.target);
  if (filters.since) {
    const cutoff = new Date(filters.since);
    actions = actions.filter(a => new Date(a.created_at) >= cutoff);
  }

  actions.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));

  if (filters.limit) actions = actions.slice(0, parseInt(filters.limit, 10));

  return actions;
}

async function getAgentActionStats(period = 'week') {
  const cutoff = getCutoffDate(period);
  const actions = readJSON(FILES.agent_actions, [])
    .filter(a => new Date(a.created_at) >= cutoff);

  const byAgent = {};
  const byType = {};

  actions.forEach(action => {
    byAgent[action.agent] = (byAgent[action.agent] || 0) + 1;
    byType[action.action_type] = (byType[action.action_type] || 0) + 1;
  });

  return {
    total: actions.length,
    by_agent: byAgent,
    by_type: byType,
    period
  };
}

// ==================== AGENT LEARNINGS ====================

async function createAgentLearning(learning) {
  const learnings = readJSON(FILES.agent_learnings, []);
  const newLearning = {
    id: generateId(),
    agent: learning.agent,
    category: learning.category,
    title: learning.title,
    content: learning.content,
    source: learning.source || null,
    verified: learning.verified || false,
    applied_count: learning.applied_count || 0,
    created_at: learning.created_at || new Date().toISOString(),
    updated_at: new Date().toISOString()
  };
  learnings.unshift(newLearning);
  writeJSON(FILES.agent_learnings, learnings);
  return newLearning;
}

async function getAgentLearnings(filters = {}) {
  let learnings = readJSON(FILES.agent_learnings, []);

  if (filters.agent) learnings = learnings.filter(l => l.agent === filters.agent);
  if (filters.category) learnings = learnings.filter(l => l.category === filters.category);
  if (filters.verified === true || filters.verified === 'true') learnings = learnings.filter(l => l.verified === true);
  if (filters.verified === false || filters.verified === 'false') learnings = learnings.filter(l => l.verified === false);
  if (filters.search) {
    const search = filters.search.toLowerCase();
    learnings = learnings.filter(l =>
      l.title.toLowerCase().includes(search) ||
      l.content.toLowerCase().includes(search)
    );
  }

  learnings.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));

  if (filters.limit) learnings = learnings.slice(0, parseInt(filters.limit, 10));

  return learnings;
}

async function updateAgentLearning(id, updates) {
  const learnings = readJSON(FILES.agent_learnings, []);
  const index = learnings.findIndex(l => l.id === id);
  if (index === -1) throw new Error('Learning not found');

  const allowed = ['title', 'content', 'verified', 'applied_count'];
  for (const key of allowed) {
    if (key in updates) learnings[index][key] = updates[key];
  }
  learnings[index].updated_at = new Date().toISOString();

  writeJSON(FILES.agent_learnings, learnings);
  return learnings[index];
}

async function incrementLearningAppliedCount(id) {
  const learnings = readJSON(FILES.agent_learnings, []);
  const index = learnings.findIndex(l => l.id === id);
  if (index === -1) throw new Error('Learning not found');

  learnings[index].applied_count = (learnings[index].applied_count || 0) + 1;
  learnings[index].updated_at = new Date().toISOString();

  writeJSON(FILES.agent_learnings, learnings);
  return learnings[index];
}

async function getAgentLearningStats() {
  const learnings = readJSON(FILES.agent_learnings, []);
  const verified = learnings.filter(l => l.verified).length;

  const byAgent = learnings.reduce((acc, row) => {
    acc[row.agent] = (acc[row.agent] || 0) + 1;
    return acc;
  }, {});

  return {
    total: learnings.length,
    verified,
    unverified: learnings.length - verified,
    by_agent: byAgent
  };
}

// ==================== MODULE EXPORTS ====================

module.exports = {
  // Tasks
  createTask,
  updateTask,
  deleteTask,
  getTasks,
  // Conversations
  addConversationSummary,
  getRecentConversations,
  // Costs
  addCostEntry,
  getCosts,
  // Agent Status
  updateAgentStatus,
  getAgentStatus,
  // Override Log
  addOverrideLog,
  getOverrides,
  // Memory Files
  upsertMemoryFile,
  getMemoryFiles,
  getMemoryFile,
  deleteMemoryFile,
  // Project Status
  ensureProjectStatusSeeded,
  getProjectStatuses,
  updateProjectStatusById,
  // Agent Actions
  createAgentAction,
  getAgentActions,
  getAgentActionStats,
  // Agent Learnings
  createAgentLearning,
  getAgentLearnings,
  updateAgentLearning,
  incrementLearningAppliedCount,
  getAgentLearningStats,
  // For health check
  isJsonFallback: true
};
